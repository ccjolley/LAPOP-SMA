...
my_data$vicbar1f[my_data$vicbar1f > 800000] <- 0
...
47% of total variance winds up in the first component.
...
Imputation decreased the fraction of variance in the first component slightly to 46%.
```{r}
pc1 <- pr[[1]]$x[,1]
pc2 <- pr[[1]]$x[,2]
qplot(pc1,pc2)
...
predict_data <- data.frame(fear10 = c(0,1), vic1ext = c(2,1),vic1exta=c(0,15),
...
All variables seem to have an effect, with a somewhat smaller one for `vicbar1f` (number of occurrences of burglaries in the neighborhood) and `fear10` (avoided walking through dangerous areas).
