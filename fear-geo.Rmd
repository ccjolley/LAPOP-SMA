...
What I want to do first is aggregate by different geographic units and see how much of the variance can be explained by each level of aggregation. First, the country level (`pais`).
...
With an R-squared of 0.13, departments are barely better than regions. Now let's get really granular and look at municipalities. Are municipality IDs unique within each country?
...
```{r}
dist_avg <- ddply(my_geo,~hondistrito,summarize,x=mean(fear))
my_geo$dist_avg <- dist_avg$x[match(my_geo$hondistrito,dist_avg$hondistrito)]
my_geo[my_geo$pais != 4,'dist_avg'] <- NA
qplot(my_geo$fear,my_geo$dist_avg)
summary(lm(fear~dist_avg,data=my_geo))
```

In Honduras, then, the availability of aldeas-level geolocation lets us explain 30% of the variance in fear.

To facilitate mapping, let's roll this up into a data frame that will contain average values of the fear index for each department in the region. We have at least some survey responses from almost every department in the region; coverage of municipalities is a lot spottier.
...
# Municipality
# TODO: I only need departments at this stage; this can all get moved further down in the Rmd
...
str_all <- paste(out_geo$country,out_geo$prov,sep='_')
str_table <- table(str_all)
out_geo <- unique(out_geo) # 53 unique departments left
...
Try this with lat/longs also and see how that looks. Incorporate the aldeas-level information in Honduras, so that results are as accurate as possible.

```{r}
# Again, from place_names.R
...
For locations in Guatemala and El Salvador, we'll get municipality locations using the Google API.
...
Just as a sanity check, let's see how far the aldeas geolocations typically are from the corresponding municipalities.

```{r}
ll <- na.omit(joined2[,c('lat','long','lat2','long2')])
ll$distance <- distCosine(ll[,c('long','lat')],ll[,c('long2','lat2')],r)
ggplot(data=ll,aes(x=distance)) +
  geom_histogram() +
  theme_classic()
```

So all aldeas-municipality displacements are within 25 miles, and most are within 5. That seems reasonable enough.
