...
The list for Guatemala is similar to Honduras, except that `vic45` is missing and we add `fear6e`
...
library(GGally)
set.seed(12345)
my_data <- lapop.2014.GTM[,c('vic40', 'vic41', 'vic44', 'vic43',
...
my_data$vicbar7f[my_data$vicbar7f > 800000] <- 0
...
predict_data <- data.frame(vic40 = c(0,1), vic41 = c(0,1), vic43 = c(0,1), 
                           vic44 = c(0,1), fear10 = c(0,1), fear6e=c(4,1),
...
From what I can see here; `vic44` doesn't have much of an effect; the others vary in size, with the `diso` variables showing the largest effect.
...
P-Values for variables that can have more than 2 values 
```{r}
...
# TODO: Find a way to visualize these
...
bin_vars <- c('vic40','vic41','vic43','vic44','vic1ext','vic1hogar',
                       'vicbar1','vicbar3','vicbar4','vicbar7')
sapply(bin_vars,function(x) bin_cor(my_data[,x]))
```

`vic44` isn't significant (as we'd expect at this point); we'll drop it and keep the rest.
