...
ggplot(data=plotme,aes(x=geo_logp,y=geo_est,color=geo_est,
  ...
  xlab('Significance (-log(p))') +
  ylab('Influence on fear index')
#TODO: Factor this out into a function as well -- might require moving a couple pieces of lm_plot() out into something else.
...
g.reg <- with(data=g_imp,exp=lm(fear_idx ~ a4_27 + a4_5 + aoj21_1 + aoj21_2 + 
                                  aoj21_7 + aoj21_8 + aut_idx + 
                                  crit_idx + d6 + dvw1 + for4_1 + guaetid2n_1 + guaetid2n_16 +
                                  it1 + leng4_1 + leng4_2 + per9 + 
                                  pn4 + pole2n + pr3a + pv_idx + pv3 + 
                                  q3c_2 + sd3new2 + tr_idx + ur + vb20_4 + vic52n + w_idx))

lm_plot(g.reg,g2,'Guatemala')
...
s.reg <- with(data=s_imp,exp=lm(fear_idx ~  aut_idx + ca_idx + clien1n + cpss1 + ed + 
                                  ed2 + elsvb48_11 + elsvb55a + elsvb55c + elsvb55g + 
                                  env1_2 + for5_10 + info1 + infra2 + it1 + np1 + 
                                  pn4 + pol1 + pole2n + pr1_3 + 
                                  q11n_6 + q3c_5 + sd6new2 + sexi + 
                                  tr_idx + ur))
lm_plot(s.reg,s2,'El Salvador')
...
h.reg <- with(data=h_imp,exp=lm(fear_idx ~ a4_22 + aut_idx + b20 + clien1na + coer1 + 
                                  ico2 + it1 + mil10c + ocup4a_3 + per4 + 
                                  pole2n + pr3e + prot3 + sd3new2 + q14 + 
                                  tr_idx + ur))

lm_plot(h.reg,h2,'Honduras')
