...
**TODO:** These methods are OK, but there are more sophisticated approaches we could try, like ANOVA and stepwise regression. I'm not sure how well these have been implemented for multiply-imputed data; that could be more of a challenge. See: http://www.statmethods.net/stats/regression.html
...
get_pvals_g <- function(i) {
  bi <- complete(g_imp,i)
  pvals <- summary(lm(fear_idx ~ .,data=bi))$coefficients[,4]  
  pvals[names(pvals) != '(Intercept)']
}
pvals <- apply(ldply(1:5,get_pvals_g),2,max)
pvals[pvals<1/ncol(g)] # apppropriate cutoff = 1/n
```
...
- `aoj21 == 8`: Biggest threat = none of the above
- `ur`: Urbanizaton
- `it1`: Interpersonal trust
- `aut_idx`: Authoritarianism index
...
```{r}
Reduce(intersect,llply(1:5,function(i) names(prune(g_imp,i,1/ncol(g)))))
```

Pruning gives us a slightly longer list, including:

- `leng4 == 1`: Parents' language (Spanish vs. indigenous)
- `a4 == 5`: Crime the biggest problem
- `vb20 == 4`: Plans to vote but leave ballot blank
- `d6`: Approval of same-sex marriage

Finally, let's see which correlations are strongest in Guatemala when we *don't* take multiple correlations into account:

```{r}
pair_pvals_g <- sort(sapply(names(g2),function(x) get_justone(g_imp,fear_gtm,x)))
pair_pvals_g[2:16]
```

In addition to the others we've seen, there's:

- `guatetid2n == 16`: Ethnic group = Q'eqchi'
- `leng1 == 201`: Native language = Spanish
- `leng1 == 207`: Native language = Q'eqchi'
- `leng4 == 3`: Parents spoke only an indigenous language
- `etid == 2`: Ladino ethnicity
- `pv3`: Believes violence was used in 2015 elections

So, in addition to the region-wide themes, the Guatemala survey shows some correlations with themes that were only really investigated there -- indigenous vs. Ladino ethnicity and election-related political violence. My intuition is that the ethnicity-related questions correlate strongly with an urban/rural split:

```{r}
summary(pool(with(data=g_imp,exp=lm(fear_idx ~ leng4_1 + leng1_201 + etid_2))))
```

So people are, on average, more fearful of crime if their parents only spoke Spanish, and if their native language is Spanish, and if they identify as Ladino.

```{r}
summary(pool(with(data=g_imp,exp=lm(fear_idx ~ ur + leng4_1 + leng1_201 + etid_2))))
```

When we add urbanization into the mix, only the parents' native language still matters. 

```{r}
summary(pool(with(data=g_imp,exp=lm(fear_idx ~ guaetid2n_1 + guaetid2n_16 + guaetid2n_22))))
summary(pool(with(data=g_imp,exp=lm(fear_idx ~ ur + guaetid2n_1 + guaetid2n_16 + guaetid2n_22))))
```

Whether we include urbanization or not, Achí (1) and Q'eqchi' (16) people are less fearful than their neighbors, while Garífuna (22) are more fearful. Of course, only `r sum(g$guaetid2n==1,na.rm=TRUE)` Achí  were surveyed at all.
