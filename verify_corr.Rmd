...
We can get to a decent set of variables using stepwise regression. What we'll do is run backward stepwise regression on each imputed dataset and keep only the ones that appear for all five sets.
...
Trying to find a good way to visualize this result... Try a plot of the estimatated coefficient against the log significance.
...
TODO: What happens with forward stepwise regression?
TODO: Incorporate muni-level averaging (as in fear-geo.Rmd) to see which hold up best independently of location.
...
# re-do imputation with location information
# TODO: can I add muni_avg into the multiple regression without adding it to the MICE data frame? That should be possible.
...
So adding in all those other variables on top of `muni_avg` increases the adjusted R^2 to 0.3, over 0.22 with `muni_avg` alone and 0.19 with `muni_avg` excluded. 

TODO: I could simplify the presentation of this by only doing the imputation and stepwise regression once -- just add `muni_avg` into what I did before and see how it impacts the significance of correlations. Which ones show the biggest changes in their (log) p-values? I could visualize this using a dot plot, maybe...
...
TODO: elsdiso18 and elsdiso19 should probably be made part of the index for SLV, given their topics and incredibly low p-values.
