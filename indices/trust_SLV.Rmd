...
With missing values, we are left with 1,354 rows.
...
All variables increase with PC1, except for `epn3a` through `epn3c`. For most variables, higher values indicate a higher level of trust in government, while the opposite is true for the `epn` variables. our index is pointing in the right direction.
