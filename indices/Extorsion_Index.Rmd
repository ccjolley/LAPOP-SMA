...
colMeans(test_data)
jitter_data <- data.frame(police=jitter(test_data$exc2,factor=2),
                          gov=jitter(test_data$exc6,factor=2),
                          soldier=jitter(test_data$exc20,factor=2),
                          muni=jitter(test_data$exc11,factor=2),
                          work=jitter(test_data$exc13,factor=2),
                          courts=jitter(test_data$exc14,factor=2),
                          health=jitter(test_data$exc15,factor=2),
                          school=jitter(test_data$exc16,factor=2))
...
As in Guatemala, not many people admit to paying bribes and correlations between different types of bribes are weak. 
...
15.4% of people reported paying a bribe to the police -- this has fairly strong correlations with paying a bribe to the municipal government (6%) or a soldier (6%).
...
my_imp
```
Of all the variables, only `exc7`needs imputation, and not as much as it did in the last two indices.
