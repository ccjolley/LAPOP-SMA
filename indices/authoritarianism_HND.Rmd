...
This index will measure the a respondant's inclination toward authoritarian political attitudes. The following variables correlate strongly with each other:
...
- `aut1`: Strong leader vs. electoral democracy (1=strong leader, 2=electoral democracy) (Honduras only)
...
- `jc16a`: President justified in dissolving Supreme Court of Justice (1=yes, 2=no) (Honduras only)
- `honjc17`: Justified for Supreme Court to remove the president (1=yes, 2=no) (Honduras only)
...
my_data <- lapop.2014.HND[,c('dem2','dem11','aut1','jc13','jc10','jc15a',
                                 'jc16a','honjc17')]
...
34% of variance is concentrated in the first component.
...
Most variables had fairly similar numbers of missing values; people were least willing to talk about coups and similar topics (`jc10`,`jc15a`,`jc16a`) and most willing to talk about mano dura policies.
...
All of our imputations are really well-correlated, and the highest-valued band is the most heavily-populated one.
...
What are we seeing here? Compared to the lowest quantile, the highest quantile of our proposed index has much lower values for `dem2` and increasing values for everything else. Recall that lower values for `dem2` indicate a preference for democracy, while higher values of the binary variables indicate less-authoritarian attitudes. Our scale in pointing in the wrong direction and needs to be reversed.
...
Let's invert, normalize and average PC1:
...
```{r,warning=FALSE}
predict_data <- data.frame(dem2=c(1,3),dem11=c(2,1),aut1=c(2,1),jc13=c(2,1),
                          jc10=c(2,1),jc15a=c(2,1),jc16a=c(2,1),
                          honjc17=c(2,1))
...
predict_data2 <- data.frame(2-diag(8))
...
While all eight variables have an impact on the composite index, `dem2` has by far the strongest influence.
...
bin_vars <- c('dem2_lo','dem2_hi','dem11','aut1','jc13','jc10','jc15a','jc16a',
              'honjc17')
ldply(bin_vars,bin_cor)
```

All are significantly-correlated (especially `dem2`); the weakest correlation is with `honjc17`.