...
geo <- c('idnum','pais','estratopri','estratosec','prov','municipio','hondistrito')
# HND has Distrito information; GTM and SLV don't
lapop.2014.SLV$hondistrito <- NA
lapop.2014.GTM$hondistrito <- NA
...
# Municipality
muni_num_hnd <- c(101,108,109,201,204,208,209,301,401,402,403,405,406,407,408,409,501,503,504,505,
                 603,604,701,702,703,704,705,805,902,905,906,907,1101,1105,1203,1307,1308,1309,1310,
                 1405,1801,1806,1807,1902,1908,2001,2101,2107,2108,2201,2205)
muni_num_gtm <- c(101,106,107,108,110,115,116,117,203,301,314,401,413,501,502,504,509,602,609,701,
                 710,806,901,911,919,920,1001,1006,1013,1104,1204,1212,1213,1301,1304,1311,1319,
                 1401,1404,1413,1504,1601,1606,1609,1614,1705,1708,1803,1805,2004,2101,2102,2201,
                 2202,2211)
muni_num_slv <- c(30101,30105,30107,30110,30201,30205,30207,30210,30302,30306,30315,30407,30408,
                  30503,30511,30512,30515,30521,30601,30602,30604,30607,30608,30610,30613,30614,
                  30617,30618,30619,30702,30703,30708,30816,30821,30822,30903,30906,31002,31003,
                  31010,31111,31117,31123,31202,31209,31217,31219,31309,31312,31403,31408,31416)
muni_text_hnd <- c('Distrito Central','El Porvenir','San Ignacio','Comayagua','Siguatepeque','El Rosario',
                   'San Jerónimo','La Paz','San Pedro Sula','Choloma','Puerto Cortés','San Manuel','Santa Cruz de Yojoa',
                   'Villanueva','La Lima','Potrerillos','El Porvenir','Tela','La Ceiba','Arizona','Sonaguera',
                   'Tocoa','Yoro','El Negrito','El Progreso','Morazán','Olanchito','José Santos Guardiola',
                   'El Paraíso','Nueva Arcadia','San José','Intibucá','Candelaria','San Juan Guarita', 
                   'Fraternidad','Arada','Ilama','Quimistán','San Pedro Zacapa ','Santa Rosa de Copán','Danlí',
                   'Yuscarán','Teupasenti','Catacamas','Guayape','Brus Laguna','Choluteca','Duyure','Pespire',
                   'Nacaome','Caridad')
muni_text_gtm <- c('Guatemala','Chinautla','San Pedro Ayampuc','Mixco','San Juan Sacatépequez','Villa Nueva',
                   'Villa Canales','Petapa','San Agustín Acasaguastlán','Antigua Guatemala','Alotenango',
                   'Chimaltenango','San Andrés Itzapa','Escuintla','Santa Lucía Cotzumalguapa','Siquinalá',
                   'San José','Barberena','Taxisco','Sololá','Panajachel','Santa María Chiquimula',
                   'Quetzaltenango','Concepción Chiquirichapa','El Palmar','Coatepeque','Mazatenango',
                   'Santo Domingo Suchitepéquez','Chicacao','San Martín Zapotitlán','Comitancillo',
                   'Nuevo Progreso','El Tumbador','Huehuetenango','Cuilco','La Libertad','Colotenango',
                   'Santa Cruz Del Quiché','Zacualpa','Nebaj','Cubulco','Cobán','Tucurú','San Pedro Carchá',
                   'Chahal','La Libertad','Dolores','El Estor','Los Amates','Jocotán','Jalapa',
                   'San Pedro Pinula','Jutiapa','El Progreso','Comapa')
muni_text_slv <- c('Ahuachapan','El Refugio','Jujutla','San Pedro Puxtla','Candelaria De La Frontera',
                   'El Porvenir','Metapan','Santa Ana','Armenia','Izalco','Sonsonate','Chalatenango',
                   'Dulce Nombre De Maria','Colon','Santa Tecla','Quezaltepeque','San Juan Opico','Tepecoyo',
                   'Aguilares','Apopa','Cuscatancingo','Ilopango','Mejicanos','Panchimalco','San Martin',
                   'San Salvador','Soyapango','Tonacatepeque','Ciudad Delgado','Cojutepeque','El Carmen',
                   'San Cristobal','San Pedro Nonualco','Zacatecoluca','San Luis La Herradura','Ilobasco',
                   'Sensuntepeque','Guadalupe','San Cayetano Istepeque','San Vicente','Mercedes Umaña',
                   'San Dionisio','Usulutan','Ciudad Barrios','Moncagua','San Miguel','Sesori','Guatajiagua',
                   'Jocoro','Concepcion De Oriente','La Union','Santa Rosa De Lima')
my_geo$muni_text <- NA
...
write.csv(my_geo,file="geo.csv")

# Get lat/longs for Honduras aldeas
...
# I still have 3210 locations with no lat/long, and the Google API
# will only give me 2500 a day for free. Fortunately, there is a much
# smaller number of unique municipalities (158).

my_geo$locstr <- paste(my_geo$muni_text,my_geo$prov_text,my_geo$pais_text,
                       sep=", ")
uniq_loc <- unique(my_geo$locstr)

library(ggmap)
latlong <- ldply(uniq_loc,function(x) geocode(x,source="dsk",output="latlon"))
...
# combine with fear index and save

fear <- read.csv('fear.csv')
join3 <- join(my_geo,fear,by="idnum")
saveme <- join3[,c('pais_text','prov_text','muni_text','dist_text','lat','long','fear')]
names(saveme) <- c('pais','prov','muni','aldea','lat','long','fear')
write.csv(saveme,file='LAPOP-geo.csv')
